...
const FALLBACK_TOPIC_COUNT = process.env.VERCEL ? 5 : 8;
...
const fallbackTopicsFromBrand = async (workspaceId, brandProfile) => {
  ...
  const unique = [...new Set(seeds)].slice(0, FALLBACK_TOPIC_COUNT);
  if (!unique.length) {
    unique.push(`${brandProfile?.name || 'Brand'} update`);
  }

  const docs = unique.map((topic, i) => ({
  ...
  try {
    const saved = await Topic.insertMany(docs, { ordered: false });
    logger.info(`Topic discovery fallback: ${saved.length} topics for workspace ${workspaceId}`);
    return saved;
  } catch (err) {
    if (err.code !== 11000) throw err;
    return Topic.find({ workspace: workspaceId, status: 'active' })
      .sort({ relevance: -1 })
      .limit(FALLBACK_TOPIC_COUNT);
  }
};
